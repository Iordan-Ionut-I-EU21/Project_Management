...
public interface CarsRepository extends JpaRepository<Cars, String> {
    ...
    @Query("SELECT COUNT(pl.car_id.id) FROM ProcessLog pl WHERE (LOWER(pl.car_id.id) = LOWER(:car_vin_or_id_or_name) " +
            "OR LOWER(pl.car_id.vin) = LOWER(:car_vin_or_id_or_name)) " +
            "OR LOWER(pl.car_id.model_id.name) = LOWER(:car_vin_or_id_or_name) AND pl.employee_id.user_id.username = :username")
    ...
    @Query("SELECT COUNT(cp.car_id.id) FROM CarParts cp WHERE (LOWER(cp.car_id.vin) = LOWER(:car_vin_or_id_or_name) " +
            "OR LOWER(cp.car_id.model_id.name) = LOWER(:car_vin_or_id_or_name) " +
            "OR LOWER(cp.car_id.id) = LOWER(:car_vin_or_id_or_name)) AND cp.installed_by.user_id.username = :username")
    ...
    @Query("SELECT COUNT(qc.car_id.id) FROM QualityChecks qc WHERE  (LOWER(qc.car_id.vin) = LOWER(:car_vin_or_id_or_name) " +
			"OR LOWER(qc.car_id.model_id.name) = LOWER(:car_vin_or_id_or_name) " +
			"OR LOWER(qc.car_id.id) = LOWER(:car_vin_or_id_or_name)) AND qc.inspector_id.user_id.username = :username")
